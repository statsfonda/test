...
et par conséquent, $P_0(|\bar{X}_n| < z_\alpha \sigma / \sqrt{n}) = 1-\alpha$. Autrement dit, sous l'hypothèse $\mu = 0$, on devrait observer l'événement 
$$ \bar{X}_n \in \left[ \pm \frac{z_\alpha \sigma}{\sqrt{n}}\right]$$
...
$$ \rejet_\alpha = \{|\bar{X}_n | > z_\alpha \sigma / \sqrt{n}\}.$$
...
\begin{align}\beta &= P_1(|\bar{X}_n|\leqslant z_\alpha \sigma / \sqrt{n}) \\
...
Cette expression ne peut pas plus se simplifier, mais on peut quand même la borner par $F(-\sqrt{n}/\sigma + z_\alpha)$. Lorsque $x$ est grand, nous avons vu (@thm-gaussiantail) que $F(x) < e^{-x^2}/|x|\sqrt{2\pi}$. Ainsi, l'erreur de première espèce est bornée par $O(e^{-n/\sigma^2/2} / \sqrt{n})$. Cela tend extrêmement vite vers 0 ; en fait, dès que $n$ est plus grand que 10 et $\sigma=1$, cette erreur est inférieure à 0.1%, donc dans ce cas le test aura une puissance supérieure à $99.9\%$. 

Que se serait-il passé si notre hypothèse alternative n'avait pas été $\mu=1$ mais $\mu = m$ pour n'importe quel $m\neq 0$ ? Dans ce cas, on aurait eu $H_1 = \mathbb{R}\setminus \{0\}$. L'erreur de première espèce aurait alors été $\beta = \sum_{m\neq 0}\beta_m $ où
$$ \beta_m = P_m(\accept_\alpha).$$
On revoyant les calculs ci-dessus, on voit que 
$$\beta_m = \Phi(-m\sqrt{n}/\sigma + z_\alpha) - \Phi(-m\sqrt{n}/\sigma + z_\alpha).$$
Il est évident $\lim_{m\to 0}\beta_m =1$, par continuité de $\Phi$. Ainsi, $\beta=1$ et $1-\beta = 0$ : pour cette seconde hypothèse alternative, la puissance de notre test… est nulle. 
