...
L'information de Fisher est la matrice de covariance du moment $T$ : 

$$I(\theta) = E_\theta[T(X)T(X)^\top] - E_\theta[T(X)]E_\theta[T(X)]^\top.$$
...
L'idée principale de l'EMV (maximiser la vraisemblance) est utilisable pour effectuer des tests. Typiquement, dans un modèle exponentiel, on peut vouloir tester l'hypothèse nulle 
$$ H_0 : \theta \in \Theta_0$$
...
$$ \left\lbrace\frac{\sup_{\theta \in \Theta_0}p_\theta(X)}{\sup_{\theta\in\Theta_1}p_\theta(X)} > z \right\rbrace. $$ 
:::
...
2. L'apparente optimalité (au sens de Cramér-Rao) de l'EMV n'est qu'asymptotique. À distance finie, il peut y avoir des estimateurs biaisés ayant un meilleur risque quadratique. 
