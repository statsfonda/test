# L'information au sens de Fisher
...
L'information de Fisher est la moyenne de la hessienne : 
$$ I(\theta) = - \mathbb{E}[\mathrm{Hessienne}\ln p_\theta(X)].$$
...
c'est-à-dire $\nabla^2_\theta p_\theta / p_\theta - s(\theta)s(\theta)^\top$. L'espérance de ceci par rapport à $p_\theta$ est donc égale à $\int \nabla^2_\theta p_\theta - I(\theta)$. Le premier terme est nul, encore par le théorème fondamental de l'analyse. 
:::
...
$\sqrt{n}(\emv - \theta)$ converge en loi vers $N(0,I(\theta))$. 
...
Pour tout estimateur sans biais $\hat{\theta}$ de $\theta$, on a $I(\theta)^{-1} \preceq \mathrm{Cov}_\theta(\hat{\theta})$. 

:::

Rappelons que, lorsque $A,B$ sont des matrices définies positives, $A \preceq B$ équivaut à ce que $\langle y, Ay\rangle \leqslant \langle y, By\rangle$ pour tout $y$. 
