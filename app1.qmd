...
2. $Py = P(tv) = tPv = t|v|^{-2}v v^\top v = t|v|^{-2}v |v|^2 = tv = y$. 
...
Le cas d'un sous-espace vectoriel généré, non pas par un unique vecteur $v$, mais par plusieurs vecteurs $v_1, \dotsc, v_d$ non-colinéaires se traite de la même façon. Soit $V = [v_1, \dotsc, v_d]$ la matrice $n \times d$ dont les colonnes sont les $v_i$. Tout à l'heure, la norme $|v|^{-1}$ aurait pu s'écrire $(v^\top v)^{-1}$. L'analogue avec $V$ est donc naturellement $(V^\top V)^{-1}$, donnant naissance au théorème suivant. 
