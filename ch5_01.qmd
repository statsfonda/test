...
Toujours sous le modèle linéaire gaussien, nous avons vu que la loi de $\left|\frac{1}{\sigma}(X^{\top} X)^{1/2}(\hat{\bt} - \bt)\right|^2 $ est une $\chi_2(d)$, et que la loi de $ \frac{(n-d)\hat{\sigma}_n^2}{\sigma^2}$
...
$$ \left\lbrace \theta \in \mathbb{R}^d : \frac{|(X^\top X)^{1/2}(\hat{\bt} - \theta) |^2 / d}{\hat{\sigma}_n^2 / (n-d)} <  f_{d,n-d,1-\alpha} \right\rbrace $$
