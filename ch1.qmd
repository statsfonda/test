...
Si $X$ est une variable aléatoire et $\mathsf{t}$ une statistique, alors $\mathsf{t}(X)$ est une variable aléatoire. On peut donc définir des quantités théoriques liées à $\mathsf{t}$: typiquement, si $X$ a pour loi $P_\theta$, on peut définir la valeur moyenne de $\mathsf{t}$ sous le modèle $P_\theta$ comme 
\begin{align*}
\mathbb{E}_\theta[\mathsf{t}(X)] = \int_\mathcal{X} \mathsf{t}(x) P_\theta(dx),
\end{align*}
