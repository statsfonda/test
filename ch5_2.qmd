...
car c'est un estimateur de la variance de $\theta_j$. 
...
$$\{ \frac{|\hat{\theta_j}|}{\hat{\sigma}_n \ell_j }> t_{n-d, 1-\alpha}\}$$
...
En multipliant des deux côtés par la matrice $[C(X^\top X)^{-1}C^\top]^{1/2}$ puis en prenant la norme au carré et en simplifiant l'expression, on voit que
$$ \frac{1}{\sigma^2}\langle C\hat{\bt}-c, C(X^\top X)^{-1}C^\top (C\hat{\bt}-c)\rangle \sim \chi^2(r).$$
Maintenant, si l'on estime le terme $\sigma^2$ comme d'habitude, on obtient la statistique de test suivante : 
$$ \frac{\langle C\hat{\bt}-c, C(X^\top X)^{-1}C^\top (C\hat{\bt}-c)\rangle /r}{\hat{\sigma}_n^2} \sim \mathscr{F}_{r, n-d}.$$


## Significativité de la régression : test de Fisher
