...
L'inégalité de Markov montre que $\mathbb{P}(S_n > t)$ est plus petit que $\mathbb{E}[e^{\lambda tS_n}]e^{-\lambda t}$. De plus, $\mathbb{E}[e^{tS_n}]$ est aussi le produit des $e^{ \varphi_i(\lambda t)}$ où $\varphi_i(t) = \ln \mathbb{E}[e^{itX_i}]$. Le lemme de Hoeffding permet donc de borner $\mathbb{P}(|S_n - \mathbb{E}[S_n]|>t)$ par 
$$ \exp\left(-\sum_{i=1}^n \frac{(b_i - a_i)^2 t^2}{8} - t\lambda\right).$$
Il suffit alors de trouver le $\lambda$ qui minimise le terme dans l'exponentielle pour trouver exactement @eq-hoeffding. 