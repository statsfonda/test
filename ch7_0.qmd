...
Quoi qu'il arrive, $|F_n(x) - F(x)|$ est plus petit que le plus grand des $|F_n(q_j) - j/N|$ augmenté de $1/N$, donc $|F_n - F|$ aussi. Pour n'importe quel $t>0$, nous pouvons utiliser la borne de l'union afin de borner $\mathbb{P}(|F_n - F|_\infty > t)$ par  $$\sum_{j}\mathbb{P}\left(\frac{1}{N} + |F_n(q_j) - j/N|>t\right).$${#eq-uniongc}
Or, $nF_n(q_j)$ suit une loi $\mathrm{Bin}(n, j/N)$, donc si^[Que se passe-t-il si $t\leqslant 1/N$ ?]  $t-1/N>0$ on peut utiliser l'inégalité de Bienaymé-Chebychev, borner la variance de $\mathrm{Bin}(n,j/N)$ par $1/4n$, et obtenir
$$\mathbb{P}\left(\frac{1}{N} + |F_n(q_j) - j/N|>t\right) \leqslant \frac{1}{4n (t - \frac{1}{N})^2}.$$
Le terme @eq-uniongc est alors plus petit que $N / 4n(t-1/N)^2$. Le choix de $N = \lceil 2/t \rceil$ (par exemple) donne 
$$\mathbb{P}(|F_n - F|_\infty > t) =O\left( \frac{1}{t^2 n}\right)$${#eq-predkw}
et donc $|F_n - F|_\infty$ tend vers 0 en probabilité, comme demandé. 
